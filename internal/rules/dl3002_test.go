...
func TestDL3002(t *testing.T) {
	...
	t.Run("can switch back to non root", func(t *testing.T) {
		dockerfile := `can switch back to non root
FROM scratch
USER root
RUN something
USER foo
DL3002`
	...
	t.Run("does not warn when switching in multiple stages", func(t *testing.T) {
		dockerfile := `does not warn when switching in multiple stages
FROM debian as base
USER root
RUN something
USER foo
...
DL3002`
	...
	t.Run("last user should not be root", func(t *testing.T) {
		dockerfile := `last user should not be root
FROM scratch
USER root
DL3002`
		violations := LintDockerfile(dockerfile, allRules)
...
DL3002`
	...
	t.Run("warns on multiple stages", func(t *testing.T) {
		dockerfile := `warns on multiple stages
FROM debian as base
USER root
RUN something
FROM scratch
...
DL3002`
	...
	t.Run("warns on transitive root user", func(t *testing.T) {
		dockerfile := `warns on transitive root user
FROM debian as base
USER root
RUN something
FROM base
